...
from utility_general import in_integer_column_span, ints_near_zero
...
class root_system:
    ...
    def __init__(self, root_list, lattice_matrix = None, full_init =  True):
        ...
        if lattice_matrix is None:
            # make the lattice_matrix a single column of zeros
            self.lattice_matrix = sp.zeros((self.vector_length, 1))
        else:
            self.lattice_matrix = lattice_matrix
        
        # check that equivalence matrix has a number of rows equal to the length of the roots
        rows, cols = self.lattice_matrix.shape
        assert(self.vector_length == rows)
    ...
    def is_root(self,vector_to_test):
        ...
        # OLD VERSION, doens't do equivalence stuff
        #return any(np.array_equal(root,vector_to_test) for root in self.root_list)
        
        # This requires checking if it is equivalant to any representative in the root_list
        # up the the lattice generated by lattice_matrix
        for root in self.root_list:
            if in_integer_column_span(vector_to_test - root, self.lattice_matrix):
                return True
        return False
    ...
    def reflect_root(self,alpha,beta):
        ...
        # Note that return values should be considered modulo the lattice L
    ...
    def is_proportional(self, alpha, beta, test_bound = 100):
        ...
        # OLD VERSION, doesn't handle lattice equivalence
        # alpha_mask = (alpha != 0)
        # beta_mask = (beta != 0)
    ...
    def is_proportional_with_constant(self, alpha, beta, test_bound = 100):
        for k in ints_near_zero(test_bound):
            if in_integer_column_span(k*alpha - beta, self.lattice_matrix):
                return True, k
        return False, None
    ...
    def verify_root_system_axioms(self, display = True):
        ...
        ##############################################################################################
        # DOES THIS NEED TO INVOLVE LATTICE EQUIVALENCE SOMEHOW?
        ##############################################################################################
        ...
        for alpha in self.root_list:
            for beta in self.root_list:
                proportional, ratio = self.is_proportional_with_constant(alpha,beta,test_bound = 10)
                if proportional: assert(ratio in (1.0,-1.0,2.0,-2.0,0.5,-0.5))
        if display: print('passed.')
