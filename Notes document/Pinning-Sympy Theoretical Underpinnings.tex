...
\begin{definition}
A \tbf{quadratic field extension} is a field extension of the form $k \lp \sqrt{d} \rp/k$ for some field $k$, where $d$ is some element of $k$ that is not already a square of an element in $k$.
\end{definition}
...
\newpage
\section{Pinnings in general}

One goal of this document is to describe similar pinning structures on special orthogonal groups and special unitary groups, as we have done for special linear groups above. This section includes various background material required to build up to that.
...
\subsection{Form, Lie algebra, torus, characters, roots}
...
\noi Let $q \ge 1$ and $n > 2q$, and let $B$ be a the matrix of a nondegenerate symmetric bilinear form on $k^n$ with Witt index $q$. The assumption $n > 2q$ ensures that $C$ is not an empty block.
\[
	B = 
	\begin{pmatrix}
		& I_q \\
...
\subsection{Split case ($n=2q$ or $n=2q+1$)}

\subsection{Quasi-split case ($n=2q+2$)}

\subsection{Non-split, non-quasi-split cases ($n > 2q + 2$)}

%
%\subsection{Pinning special orthogonal groups}
%
%For the special orthogonal group $\SO_{n,q}(k,b)$, with root system $\Phi$, we describe a map $X_\alpha$ for every root $\alpha \in \Phi$ with all the various pinning properties.
%
...
In this section, we study pinnings of special unitary groups. For the duration of this section, fix a quadratic field extension $L/k = k(\sqrt{d})/k$ and let $\sig(x) = \ov x$ be the standard involution
\[
	\ov{a+b \sqrt{d}} = a-b \sqrt{d}
\]
(for $a,b \in k$). Also fix $n$, and fix a hermitian or skew-hermitian form $h$ on $V = L^{n}$. Fix a basis of $V$, and let $H$ be the matrix of $h$ in the basis $\beta$. Let $q$ be the Witt index of $h$. Via a ``hermitian version" of the Witt decomposition theorem, it is possible to choose a basis of $V$ so that the matrix $H$ has the form
\[
	H =
	\begin{pmatrix}
		& I_q \\
...
H =
	...
	\end{pmatrix}
\]
Note that $h$ has Witt index $q$, half the dimension of $V$. Since t Witt index can never be more than half the dimension, is equivalent to saying that the Witt index is maximal. The associated special unitary group to $h$ (and $H$) is
\[
...
\begin{example}
Consider the special case $(n,q) = (4,2)$. The root system is type $\mathsf{C}_2$.
\[
...
\begin{example}
Consider the smallest non-quaisi-split special unitary group of interest, the case $(n,q) = (5,2)$. In this case, $V$ is a 5-dimensional $L$-vector space, and $h$ is the hermitian or skew-hermitian form on $V$ with matrix
\[
	H =
	\begin{pmatrix}
		& I_2 \\
